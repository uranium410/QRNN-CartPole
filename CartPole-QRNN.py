...
class Network:
    def __init__(self, learning_rate=0.01, state_size=4,action_size=2, hidden_size=10):
        with tf.name_scope('Model') as scope:
            ...
            self.model.add(QRNN(hidden_size, window_size=3, dropout = 0))
    ...
    def replay(self, memory, batch_size, gamma, targetQN ,state_size = 4, dim_size = 3):
        inputs = np.zeros((batch_size, state_size, dim_size))
        targets = np.zeros((batch_size, 2))
        mini_batch = memory.sample(batch_size)

        for i, (state_b, action_b, reward_b, next_state_b) in enumerate(mini_batch):
            state_b = np.reshape(state_b, [1, 4, 3])
            next_state_b = np.reshape(next_state_b, [1, 4, 3])
...
callbacks.TensorBoard(log_dir="~/tflog/", histogram_freq=1)
...
#-------------Env Settings----------------#
    ...
    f = open('results/QRNN/result' + fileNumber + '.csv', 'w', newline="")
    writer = csv.writer(f)
    ...
    for episode in range(num_episodes):
        ...
        state1 = np.reshape(state, [1,4,1])
        state2 = state1
        state3 = state2

        state = np.concatenate([state1,state2,state3], axis=2)
        ...
        for t in range(2, max_number_of_steps + 1):
            ...
            state1 = np.reshape(state1, [1,4,1])
            
            next_state =  np.concatenate([state1,state2,state3], axis=2)
